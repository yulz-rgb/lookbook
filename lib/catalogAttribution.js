...
function preferCatalogField(stored, fallback) {
...
export function enrichProductsWithDefaults(stored = [], defaults = []) {
  ...
  return stored.map((p) => {
    ...
    return normalizeUniformProduct({
      ...
      fabric: preferCatalogField(p.fabric, ref.fabric),
      sizeRange: preferCatalogField(p.sizeRange, ref.sizeRange),
      details: preferCatalogField(p.details, ref.details),
    });
  });
}
