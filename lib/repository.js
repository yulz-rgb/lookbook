...
import { mergeCatalogWithDefaults, ensureFullBundledCatalog } from './catalogAttribution.js';
...
async function ensureSupplier(yachtId, supplierName) {
  const name = String(supplierName || '').trim();
  if (!name) return null;
  const db = getDb();
  return db.supplier.upsert({
    where: { yachtId_name: { yachtId, name } },
    update: {},
    create: { yachtId, name },
  });
}
...
export async function getWorkspace(yachtId) {
  ...
  const merged = mergeCatalogWithDefaults(products.map(serializeProduct), defaultProducts);
  const catalog = ensureFullBundledCatalog(merged, defaultProducts);
  return {
    products: catalog,
...
export async function upsertProduct(yachtId, input, id) {
  const db = getDb();
  const supplier = await ensureSupplier(yachtId, input.supplierName || input.brand);
  const normalized = normalizeUniformProduct(input);
  const colourImages = normalized.colourImages && Object.keys(normalized.colourImages).length
    ? normalized.colourImages
    : undefined;
  const data = {
    ...
    swatch: normalized.swatch,
    accent: normalized.accent,
    fabric: cleanFabricComposition(normalized.fabric || ''),
    ...
    imageHint: normalized.imageHint,
...
export async function applyCatalogImport(yachtId, validatedRows, filename, userId, { replace = false } = {}) {
  const db = getDb();
  let created = 0;
  let updated = 0;
  const errors = [];
  ...
  if (replace) {
    await db.lookItem.deleteMany({ where: { look: { yachtId } } });
    await db.product.deleteMany({ where: { yachtId } });
  }
    ...
    for (let i = 0; i < products.length; i += 1) {
      ...
      await tx.product.create({
        data: {
          ...
          fabric: cleanFabricComposition(p.fabric || '') || null,
          ...
          colourImages: p.colourImages && Object.keys(p.colourImages).length ? p.colourImages : undefined,
