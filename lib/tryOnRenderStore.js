...
let preferMemoryStore = false;

function isMissingTryOnTableError(err) {
  return err?.code === 'P2021' && String(err?.meta?.table || '').includes('TryOnRender');
}

function shouldUseMemoryStore(err) {
  if (isMissingTryOnTableError(err)) {
    preferMemoryStore = true;
    return true;
  }
  return false;
}

function readMemoryByCacheKey(cacheKey) {
  const row = memoryRenders.get(cacheKey);
  return row ? serializeRender(row) : null;
}

function readMemoryById(renderId) {
...
export async function findRenderByCacheKey(cacheKey) {
  if (hasDatabase && !preferMemoryStore) {
    try {
      const db = getDb();
      const row = await db.tryOnRender.findUnique({ where: { cacheKey } });
      return row ? serializeRender(row) : null;
    } catch (err) {
      if (!shouldUseMemoryStore(err)) throw err;
    }
  }
  return readMemoryByCacheKey(cacheKey);
}
...
export async function findRenderById(renderId) {
  ...
  return readMemoryById(renderId);
}
