...
import { productMatchesBodyType } from './catalog.js';
import {
  crewEligibleForItem,
  itemBaseQty,
  itemOrderQty,
  looksUseItemAllocations,
  normalizeLookItems,
} from './lookAllocation.js';

const CURRENCY_SYMBOLS = { EUR: '\u20ac', USD: '$', GBP: '\u00a3' };

const TOP_CATEGORIES = new Set(['tops', 'shirts', 'dresses', 'outerwear', 'accessories', 'epaulettes', 'chef-wear', 'engineering', 'spa-wear']);
...
export function buildLookTotals(looks = [], products = []) {
  ...
  return looks.map((look) => {
    ...
    const items = lookProducts(normalized, byId);
    return {
      ...normalized,
      products: items,
...
function lookAllocationEntries(look, productsById) {
...
function aggregateAllocatedQuantities(crew = [], looks = [], products = [], settings = {}) {
  const productsById = indexById(products);
  const lookTotals = Array.isArray(looks) && looks[0]?.products
    ? looks
    : buildLookTotals(looks, products);
  const totals = new Map();

  for (const look of lookTotals) {
    for (const { item, product } of lookAllocationEntries(look, productsById)) {
      const baseQty = itemBaseQty(crew, look, item, settings);
      const spareQty = Math.max(0, num(item.spareQty, 0));
      const existing = totals.get(product.id) || { product, baseQty: 0, spareQty: 0 };
      existing.baseQty += baseQty;
      existing.spareQty += spareQty;
...
export function computeCrewRows(crew = [], lookTotals = [], settings = {}) {
  ...
  const useAllocations = looksUseItemAllocations(lookTotals);
    ...
    if (useAllocations) {
      for (const look of assigned) {
        for (const item of look.items || []) {
          if (!crewEligibleForItem(member, look, item)) continue;
          const product = (look.products || []).find((p) => p.id === item.productId);
          if (!product) continue;
          const units = num(item.unitsPerPerson, 1);
          garmentPerSet += num(product.price) * units;
          logoPerSet += units * s.logoCost;
          vatPerSet += num(product.price) * units * (num(product.vatRate) / 100);
          itemCount += units;
        }
      }
    } else {
      ...
      garmentPerSet = assigned.reduce((sum, l) => sum + num(l.subtotal), 0);
      logoPerSet = itemCount * s.logoCost;
      vatPerSet = products.reduce(
...
export function computeBudget(crew = [], lookTotals = [], settings = {}) {
  ...
  let spareTotal = 0;
  let spareVatTotal = 0;
  let spareUnitCount = 0;
  if (useAllocations) {
    const productsById = indexById(lookTotals.flatMap((look) => look.products || []));
    for (const look of lookTotals) {
      for (const item of look.items || []) {
        const product = productsById[item.productId];
        if (!product) continue;
        const spareQty = Math.max(0, num(item.spareQty, 0));
        spareUnitCount += spareQty;
        spareTotal += spareQty * (num(product.price) + s.logoCost);
        spareVatTotal += spareQty * num(product.price) * (num(product.vatRate) / 100);
      }
    }
  } else {
    spareTotal = baseTotal * (s.sparePercent / 100);
  }

  const vatTotal = rows.reduce((sum, r) => sum + r.vatTotal, 0) + spareVatTotal;
  ...
  return {
    ...
    currency: s.currency,
    rows,
    usesItemAllocations: useAllocations,
  };
}
...
export function buildOrderSummary(crew = [], looks = [], products = [], settings = {}) {
  ...
  if (useAllocations) {
    for (const [productId, entry] of aggregateAllocatedQuantities(crew, looks, products, settings).entries()) {
      counts.set(productId, { baseQty: entry.baseQty, spareQty: entry.spareQty });
    }
  } else {
    const lookById = indexById(lookTotals);
    for (const member of crew) {
      ...
      for (const look of memberLooks(member, lookById, lookTotals[0])) {
        for (const product of look.products) {
          const existing = counts.get(product.id) || { baseQty: 0, spareQty: 0 };
          existing.baseQty += sets;
          counts.set(product.id, existing);
        }
      }
    }
  }
  ...
  const lines = [];
  for (const [productId, qtys] of counts.entries()) {
    const product = productsById[productId];
    if (!product) continue;
    const baseQty = qtys.baseQty;
    const withSpare = useAllocations
      ? baseQty + qtys.spareQty
      : Math.ceil(baseQty * (1 + s.sparePercent / 100));
...
export function buildSizeAwareOrderSummary(crew = [], looks = [], products = [], settings = {}) {
  ...
  for (const member of crew) {
    const sets = memberSets(member, s);
    for (const look of memberLooks(member, lookById, lookTotals[0])) {
      const entries = useAllocations
        ? (look.items || [])
          .filter((item) => crewEligibleForItem(member, look, item))
          .map((item) => ({ item, product: productsById[item.productId] }))
          .filter((entry) => entry.product)
        : (look.products || []).map((product) => ({
          item: { unitsPerPerson: 1, spareQty: 0 },
          product,
        }));
      ...
      for (const { item, product } of entries) {
        ...
        const units = useAllocations ? num(item.unitsPerPerson, 1) : 1;
        existing.baseQty += units * sets;
  ...
  if (useAllocations) {
    const spareByProduct = new Map();
    for (const look of lookTotals) {
      for (const item of look.items || []) {
        spareByProduct.set(
          item.productId,
          (spareByProduct.get(item.productId) || 0) + Math.max(0, num(item.spareQty, 0)),
        );
      }
    }
    const assignedSpare = new Set();
    for (const line of lineMap.values()) {
      if (!spareByProduct.has(line.productId) || assignedSpare.has(line.productId)) continue;
      line.spareQty = spareByProduct.get(line.productId);
      assignedSpare.add(line.productId);
    }
  }
  ...
  for (const line of lineMap.values()) {
    const withSpare = useAllocations
      ? line.baseQty + line.spareQty
      : Math.ceil(line.baseQty * (1 + s.sparePercent / 100));
    lines.push({
      ...line,
      orderQty: withSpare,
  ...
  for (const [name, count] of nameCounts.entries()) {
    if (count > 1) {
      warnings.push({
        ...
        message: `"${name}" appears on ${count} products.`,
      });
    }
  }
  ...
  for (const member of crew) {
    ...
    for (const look of assigned) {
      for (const product of look.products) {
        ...
        if (body && !productMatchesBodyType(product, body)) {
          warnings.push({
            level: 'warning',
            code: 'FIT_MISMATCH',
