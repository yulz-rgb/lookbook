...
/* --- External store for useSyncExternalStore (avoids setState-in-effect on load) --- */

const EMPTY_ADJUSTMENTS = {};
let adjustmentsSnapshot = null;
const adjustmentListeners = new Set();

export function subscribePreviewAdjustments(listener) {
  adjustmentListeners.add(listener);
  return () => adjustmentListeners.delete(listener);
}

export function getPreviewAdjustmentsSnapshot() {
  if (typeof window === 'undefined') return EMPTY_ADJUSTMENTS;
  if (adjustmentsSnapshot === null) adjustmentsSnapshot = loadPreviewAdjustments();
  return adjustmentsSnapshot;
}

export function getPreviewAdjustmentsServerSnapshot() {
...
function persistAdjustments(all) {
  ...
  // Publish the new (fresh-reference) snapshot so subscribed components re-render.
  adjustmentsSnapshot = all;
  for (const listener of adjustmentListeners) listener();
}
