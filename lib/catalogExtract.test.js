...
describe('isUniformCatalogRecord', () => {
  ...
  it('drops non-uniform supplier lines', () => {
    const excluded = [
      ...
      'Sun Bum SPF 70 Clear Face Lotion Travel Size',
      'Sun Bum After Sun Cool Down Gel',
      'Project Reef SPF 50 Reef-Safe Sunstick',
      'Project Reef After-Sun Cooling Lotion',
    ];
    for (const name of excluded) {
      expect(isUniformCatalogRecord({ name })).toBe(false);
    }
    expect(isUniformCatalogRecord({
      name: 'Canvas Pouch',
      productUrl: 'https://www.liquidyachtwear.com/shop/guest/suncare/canvas-pouch/',
    })).toBe(false);
  });
});
