...
describe('shopifyProductToRecord', () => {
  it('maps Shopify product JSON to catalog rows', async () => {
    ...
    const row = shopifyProductToRecord({
      ...
      variants: [{ sku: 'BC-LAD-001', price: '8.50', title: 'White / S' }],
      images: [{ src: 'https://cdn.example.com/shirt.png' }],
    ...
    expect(row.fabric).toContain('cotton');
    expect(row.imageUrl).toContain('shirt.png');
