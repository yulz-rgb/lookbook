...
import { guessFitFromProductTitle } from './catalogExtract.js';
...
/** Effective fit for catalog filtering — title/category override bad or missing fit data. */
export function resolveProductFit(product) {
  const name = String(product?.name || '');
  const category = product?.category || '';
  if (/\bunisex\b/i.test(name)) return ['woman', 'man'];

  const fromTitle = guessFitFromProductTitle(name, category);
...
export function productMatchesBodyType(product, bodyType) {
  if (!bodyType || !VALID_BODY_TYPES.has(bodyType)) return true;
  return resolveProductFit(product).includes(bodyType);
}
