...
describe('catalogAttribution', () => {
  ...
  it('restores fabric and sizeRange when server rows saved empty catalog fields', () => {
    const stored = [{
      id: 'marina-polo',
      name: 'Polo',
      fabric: '',
    ...
    const defaults = [{
      ...
      sizeRange: 'XS–XL',
      details: 'Fabric: 95% cotton 5% elastane Sizes: XS - XL',
    }];
    const [enriched] = enrichProductsWithDefaults(stored, defaults);
    expect(enriched.fabric).toBe('95% cotton & 5% elastane');
    expect(enriched.sizeRange).toBe('XS–XL');
    expect(enriched.details).toContain('Fabric:');
  });
