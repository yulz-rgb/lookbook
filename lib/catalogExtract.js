...
function rowFromParts({
  name, price, currency, brand, sku, category, imageUrl, details,
  fabric = '', colours = '', fit = 'woman|man', sizeRange = '',
  supplierName = '', productUrl = '',
}) {
...
export function guessFitFromProductTitle(title) {
...
export function shopifyProductToRecord(product, supplierDefault = '', origin = '') {
  ...
  const imageUrl = product.images?.[0]?.src
    || variants.find((v) => v.featured_image?.src)?.featured_image?.src
    || '';

  const sku = variants.find((v) => v.sku)?.sku || product.handle || '';
  const brand = product.vendor || '';
  const context = `${product.product_type || ''} ${plainDetails.slice(0, 400)}`;
  const category = guessCategory(title, context);
  const fit = guessFitFromProductTitle(title).join('|');
