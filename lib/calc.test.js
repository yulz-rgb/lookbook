...
const products = [
  { id: 'a', name: 'Polo', price: 10, vatRate: 10, minOrder: 6, category: 'tops', fit: ['woman', 'man'], colours: ['White'] },
  { id: 'b', name: 'Shorts', price: 20, vatRate: 0, minOrder: 2, category: 'bottoms', fit: ['woman'] },
];
const looks = [{
  id: 'L1',
  name: 'Look 1',
  ...
  productIds: ['a', 'b'],
  items: [
    { productId: 'a', unitsPerPerson: 1, roleIds: ['interior'], spareQty: 1 },
    { productId: 'b', unitsPerPerson: 1, roleIds: ['interior'], spareQty: 4 },
  ],
}];
const crew = [
...
describe('computeBudget', () => {
  ...
  it('computes line components', () => {
    ...
    // Spares: 1 polo + 4 shorts = garment 10+80=90, logo 5+20=25
    expect(b.spareTotal).toBe(115);
    expect(b.spareUnitCount).toBe(5);
    expect(b.vatTotal).toBeCloseTo(3, 5);
    expect(b.shippingTotal).toBe(50);
    expect(b.setupTotal).toBe(100);
  });
  it('computes grand total including VAT, shipping and setup', () => {
    expect(b.grandTotal).toBeCloseTo(348, 5);
  });
...
describe('buildOrderSummary', () => {
  ...
  it('aggregates quantity across sets with spare and flags MOQ', () => {
    ...
    expect(bLine.orderQty).toBe(6);
    expect(bLine.meetsMoq).toBe(true);
    expect(bLine.lineTotal).toBe(120);
  });
});
